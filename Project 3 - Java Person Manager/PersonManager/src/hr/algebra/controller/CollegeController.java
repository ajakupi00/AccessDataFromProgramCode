...
public class CollegeController implements Initializable {
    ...
    private void commit(ActionEvent event) {
        if (studentFormValid()) {
            ...
            if (selectedStudentViewModel.getStudent().getId() == 0) {
                ...
                try {
                    RepositoryFactory.getRepository().addStudent(selectedStudentViewModel.getStudent());
                    tvStudents.refresh();
                } catch (Exception ex) {
                    Logger.getLogger(CollegeController.class.getName()).log(Level.SEVERE, null, ex);
                }
    ...
    private void deleteStudent(ActionEvent event) {
        if(tvStudents.getSelectionModel().getSelectedItem() != null){
            try {
                RepositoryFactory.getRepository().deleteStudent(tvStudents.getSelectionModel().getSelectedItem().getStudent());
    ...
    private void initStudentsTable() {
        ...
        tcStudentJmbag.setCellValueFactory(s -> s.getValue().getJmbagProperty());
    ...
    private void bindStudent(StudentViewModel viewModel) {
        ...
        tfJmbag.setText(selectedStudentViewModel.getJmbagProperty().get());
